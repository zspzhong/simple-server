...
function init() {
    ...
    function _ipFilter() {
        ...
        app.use('/', function (req, res, next) {
            var clientIp = (req.headers['x-real-ip'] || req.ip || '').replace('::ffff:', '');
            if (_.includes(global.whiteList, clientIp)) {
                next(null);
                return;
            }

            logger.info('Forbidden visit from', clientIp);
            next(403);
        });
    }
    ...
    function _middleware() {
        ...
        _.each(global.appOption.middleware, function (item) {
            ...
            try {
                ...
                app.use(item.path ? '/' : item.path, require(appPath));
            }
            catch (e) {
                try {
                    serverPath = require.resolve(serverPath);
                    app.use(item.path ? '/' : item.path, require(serverPath));
                }
                catch (e) {
