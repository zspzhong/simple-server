...
function init() {
    ...
    function _ip() {
        app.use('/', function (req, res, next) {
            req.clientIp = (req.headers['x-real-ip'] || req.ip || '').replace('::ffff:', '');
            next(null);
        });
    }
    ...
    function _ipFilter() {
        ...
        app.use('/', function (req, res, next) {
            if (_.includes(global.whiteList, req.clientIp)) {
                next(null);
                return;
            }

            logger.info('Forbidden visit from', req.clientIp);
            next(403);
        });
    }
        ...
        _.each(global.appOption.middleware, function (item) {
            ...
            try {
                ...
                try {
                    ...
                    logger.warn('middleware is not found', item, e);
                }
            }
        });
    }
