...
function Application(appName, metaPath, expressApp) {
    ...
    this.metaInfo = _prefix(this.metaInfo);

    function _prefix(metaInfo) {
        var prefix = (global.appOption.servicePrefix ? '/svc/' + global.appOption.servicePrefix : '/svc');
        _.each(metaInfo.service, function (item) {
...
Application.prototype.loadMiddleware = function () {
    _.each(this.metaInfo.service, function (item) {
