...
function get(url, options, callback) {
    ...
    request(options, function (err, res, result) {
        if (err) {
            logger.error('inner request GET error', options.uri);
            callback(err);
            return;
        }

        logger.info('inner request GET', options.uri, res.statusCode, (Date.now() - start) + 'ms');
...
function post(url, data, options, callback) {
    ...
    request(options, function (err, res, result) {
        if (err) {
            logger.error('inner request POST error', options.uri);
            callback(err);
            return;
        }

        logger.info('inner request POST', options.uri, res.statusCode, (Date.now() - start) + 'ms');
